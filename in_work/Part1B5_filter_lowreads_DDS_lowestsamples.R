...
f_filter_counts <- function(cts, dpi_lowest, cutoff_count, cutoff_percentage) {
  # Select for only P1 samples by substring
  samples_oi <- grep(paste0("^t", dpi_lowest), colnames(cts))
...
# rename based on lowest dpi, contrast
names(flat_filter_list) <- paste0(paste0("dpi", 
                                         lowest_dpi, 
                                         "_"
                                         ), 
                                  sapply(flat_filter_list, 
                                         function(x) 
                                           x$name
                                         )
                                  )
...
# if the number of retained genes is too low it doesn't work!
# dds_list_vst <- length()

names_dds <- names(dds_list)

f_vst_list <- function(dds, name) {
  cat("Processing element:", name, "\n")
  
  result <- try(vst(dds, fitType = "local"), silent = TRUE)
  
  if (inherits(result, "ERROR")) {
    cat("Error in element:", name, "\n")
  }
  return(result)
}
  
cts_vst_list <-  map2(.x = dds_list,
                      .y = names_dds,
                      .f = f_vst_list)

cts_vst_list <- cts_vst_list[-length(cts_vst_list)]
names_dds <- names(dds_list)[-length(names(dds_list))]

# Define the function to extract VST count matrix
f_vst_matrix <- function(vst_result, name) {
  if (!is.null(vst_result)) {
    cat("Extracting VST count matrix for:", name, "\n")
    return(assay(vst_result))
  } else {
    cat("Skipping extraction for:", name, "\n")
    return(NULL)
  }
}

# Extract VST count matrices
vst_count_matrices <- map2(.x = cts_vst_list, 
                           .y = names_dds, 
                           .f = f_vst_matrix
                           )


# # function to store dispersion plots
# f_dispers_plot <- function(dds, name){
#   cat("Processing_file:", name)
#   
#   p_disp <- plotDispEsts(dds_one)
#   return(p_disp)
#   
# }
#   
# p_list_disp <- map2(.x = dds_list,
#                     .y = names_dds,
#                     .f = f_dispers_plot)
# 
# ### No saving possible yet
...
# Save DESEq2 normalised counts
map2(.x = vst_count_matrices,
     .y = paste0("VST_", names_dds),
     .f = f_safe_df,
     dir = dir_out,
     gois = "",
     part = "P1B4")
